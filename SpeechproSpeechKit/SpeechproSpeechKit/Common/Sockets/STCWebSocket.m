...
typedef NS_ENUM(NSUInteger, STCOpCode) {
    STCOpCodeContinueFrame = 0x0,
    STCOpCodeTextFrame = 0x1,
    STCOpCodeBinaryFrame = 0x2,
    //3-7 are reserved.
    STCOpCodeConnectionClose = 0x8,
    STCOpCodePing = 0x9,
    STCOpCodePong = 0xA,
    //B-F reserved.
};

typedef NS_ENUM(NSUInteger, STCCloseCode) {
    STCCloseCodeNormal                 = 1000,
    STCCloseCodeGoingAway              = 1001,
    STCCloseCodeProtocolError          = 1002,
    STCCloseCodeProtocolUnhandledType  = 1003,
    // 1004 reserved.
    STCCloseCodeNoStatusReceived       = 1005,
    //1006 reserved.
    STCCloseCodeEncoding               = 1007,
    STCCloseCodePolicyViolated         = 1008,
    STCCloseCodeMessageTooBig          = 1009
};

typedef NS_ENUM(NSUInteger, STCInternalErrorCode) {
    // 0-999 WebSocket status codes not used
    STCOutputStreamWriteError  = 1
};

#define kSTCInternalHTTPStatusWebSocket 101

//holds the responses in our read stack to properly process messages
@interface STCResponse : NSObject

@property(nonatomic, assign) BOOL isFin;
@property(nonatomic, assign) STCOpCode code;
...
@interface STCWebSocket ()<NSStreamDelegate>
...
static const uint8_t STCFinMask             = 0x80;
static const uint8_t STCOpCodeMask          = 0x0F;
static const uint8_t STCRSVMask             = 0x70;
static const uint8_t STCMaskMask            = 0x80;
static const uint8_t STCPayloadLenMask      = 0x7F;
static const size_t  STCMaxFrameSize        = 32;

@implementation STCWebSocket
...
- (void)disconnect {
    [self writeError:STCCloseCodeNormal];
}

- (void)writeString:(NSString*)string {
    if(string) {
        [self dequeueWrite:[string dataUsingEncoding:NSUTF8StringEncoding]
                  withCode:STCOpCodeTextFrame];
    }
}

- (void)writePing:(NSData*)data {
    [self dequeueWrite:data withCode:STCOpCodePing];
}

- (void)writeData:(NSData*)data {
    [self dequeueWrite:data withCode:STCOpCodeBinaryFrame];
}
...
//                                     (__bridge CFStringRef)[NSString stringWithFormat:@"%@:%@", self.url.host, port]);
    ...
    NSLog(@"%@",[NSString stringWithFormat:@"%@:%@", self.url.host, port]);
    CFHTTPMessageSetHeaderFieldValue(urlRequest,
...
- (BOOL)validateResponse:(uint8_t *)buffer length:(NSInteger)bufferLen responseStatusCode:(CFIndex*)responseStatusCode {
    ...
    *responseStatusCode = CFHTTPMessageGetResponseStatusCode(response);
    BOOL status = ((*responseStatusCode) == kSTCInternalHTTPStatusWebSocket)?(YES):(NO);
    if(status == NO) {
        CFRelease(response);
        return NO;
    }
...
-(void)processRawMessage:(uint8_t*)buffer length:(NSInteger)bufferLen {
    STCResponse *response = [self.readStack lastObject];
    ...
    if(response.bytesLeft > 0) {
        ...
        BOOL isFin = (STCFinMask & buffer[0]);
        uint8_t receivedOpcode = (STCOpCodeMask & buffer[0]);
        BOOL isMasked = (STCMaskMask & buffer[1]);
        uint8_t payloadLen = (STCPayloadLenMask & buffer[1]);
        NSInteger offset = 2; //how many bytes do we need to skip for the header
        if((isMasked  || (STCRSVMask & buffer[0])) && receivedOpcode != STCOpCodePong) {
            [self doDisconnect:[self errorWithDetail:@"masked and rsv data is not currently supported" code:STCCloseCodeProtocolError]];
            [self writeError:STCCloseCodeProtocolError];
            return;
        }
        BOOL isControlFrame = (receivedOpcode == STCOpCodeConnectionClose || receivedOpcode == STCOpCodePing);
        if(!isControlFrame && (receivedOpcode != STCOpCodeBinaryFrame && receivedOpcode != STCOpCodeContinueFrame
                               && receivedOpcode != STCOpCodeTextFrame && receivedOpcode != STCOpCodePong)) {
            [self doDisconnect:[self errorWithDetail:[NSString stringWithFormat:@"unknown opcode: 0x%x",receivedOpcode] code:STCCloseCodeProtocolError]];
            [self writeError:STCCloseCodeProtocolError];
            return;
        }
        if(isControlFrame && !isFin) {
            [self doDisconnect:[self errorWithDetail:@"control frames can't be fragmented" code:STCCloseCodeProtocolError]];
            [self writeError:STCCloseCodeProtocolError];
            return;
        }
        if(receivedOpcode == STCOpCodeConnectionClose) {
            //the server disconnected us
            uint16_t code = STCCloseCodeNormal;
            if(payloadLen == 1) {
        ...
        if(receivedOpcode == STCOpCodePong) {
        ...
        if(!isFin && receivedOpcode == STCOpCodeContinueFrame && !response) {
            [self doDisconnect:[self errorWithDetail:@"continue frame before a binary or text frame" code:STCCloseCodeProtocolError]];
            [self writeError:STCCloseCodeProtocolError];
            return;
        }
        BOOL isNew = NO;
        if(!response) {
            if(receivedOpcode == STCOpCodeContinueFrame) {
                [self doDisconnect:[self errorWithDetail:@"first frame can't be a continue frame" code:STCCloseCodeProtocolError]];
                [self writeError:STCCloseCodeProtocolError];
                return;
            }
            isNew = YES;
            response = [STCResponse new];
            ...
            if(receivedOpcode == STCOpCodeContinueFrame) {
                ...
                [self doDisconnect:[self errorWithDetail:@"second and beyond of fragment message must be a continue frame" code:STCCloseCodeProtocolError]];
                [self writeError:STCCloseCodeProtocolError];
                return;
            }
...
- (BOOL)processResponse:(STCResponse*)response {
    if(response.isFin && response.bytesLeft <= 0) {
        NSData *data = response.buffer;
        if(response.code == STCOpCodePing) {
            [self dequeueWrite:response.buffer withCode:STCOpCodePong];
        } else if(response.code == STCOpCodeTextFrame) {
            NSString *str = [[NSString alloc] initWithData:response.buffer encoding:NSUTF8StringEncoding];
            if(!str) {
                [self writeError:STCCloseCodeEncoding];
                return NO;
            }
            __weak typeof(self) weakSelf = self;
            dispatch_async(self.queue,^{
...
-(void)dequeueWrite:(NSData*)data withCode:(STCOpCode)code {
    if(!self.isConnected) {
        return;
    }
    if(!self.writeQueue) {
    ...
    [self.writeQueue addOperationWithBlock:^{
        ...
        NSMutableData *frame = [[NSMutableData alloc] initWithLength:(NSInteger)(dataLength + STCMaxFrameSize)];
        uint8_t *buffer = (uint8_t*)[frame mutableBytes];
        buffer[0] = STCFinMask | code;
        if(dataLength < 126) {
            buffer[1] |= dataLength;
        ...
        while (true) {
            ...
            if(len < 0 || len == NSNotFound) {
                ...
                if(!error) {
                    error = [strongSelf errorWithDetail:@"output stream error during write" code:STCOutputStreamWriteError];
                }
...
- (NSError*)errorWithDetail:(NSString*)detail code:(NSInteger)code userInfo:(NSDictionary *)userInfo {
    ...
    return [[NSError alloc] initWithDomain:@"STCWebSocket" code:code userInfo:details];
}

- (void)writeError:(uint16_t)code {
    uint16_t buffer[1];
    buffer[0] = CFSwapInt16BigToHost(code);
    [self dequeueWrite:[NSData dataWithBytes:buffer length:sizeof(uint16_t)] withCode:STCOpCodeConnectionClose];
}
